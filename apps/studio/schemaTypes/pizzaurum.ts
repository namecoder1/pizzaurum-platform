...
export const pizzaurum = defineType({
      ...
      validation: (Rule) => [
        Rule.custom(async (value, context) => {
          ...
          try {
            ...
            // Query per contare le pizze mensili esistenti (escludendo il documento corrente sia come draft che pubblicato)
            const query = `
              count(
                *[
                  _type == "pizzaurum" && 
            ...
            const currentMonthlyCount = await client.fetch(query, {
            ...
            // Permetti massimo 1 pizza mensile
            if (currentMonthlyCount >= 1) {
              return 'Puoi avere solo una pizza mensile. Disattiva prima la pizza mensile esistente.'
            }

            return true
          } catch (error) {
            console.error('Errore nella validazione pizza mensile:', error)
